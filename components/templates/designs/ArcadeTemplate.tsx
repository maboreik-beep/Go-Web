...
// New Template: Foundry (for Construction/Interior Design)
export const ArcadeTemplate: React.FC<TemplateProps> = (props) => {
    ...
    return (
        <TemplateWrapper {...props}>
            <div className="bg-white">
                {isVisible('hero') && (
                    <section className="relative h-[80vh] flex items-end p-12 text-white bg-gray-800"
                        ...
                        <div className="max-w-3xl">
                            <h2 className="text-5xl md:text-7xl font-extrabold mb-4 uppercase tracking-wider">{getText(c.hero.headline)}</h2>
                            <p className="text-lg mb-8 opacity-90">{getText(c.hero.subheadline)}</p>
                            <button className="preview-primary-bg text-white font-bold py-4 px-10 text-lg hover:opacity-90">{getText(c.hero.ctaButton)}</button>
                        </div>
                    </section>
                )}
                ...
                {isVisible('services') && (
                    <section className="py-24 px-6 bg-gray-50">
                        <div className="container mx-auto">
                            <h3 className="text-4xl font-bold mb-16 text-center text-gray-800">{getText(c.services.title)}</h3>
                            <div className="grid md:grid-cols-3 gap-1">
                                {c.services.items.map((item, index) => (
                                    <div key={index} className="relative group overflow-hidden h-96">
                                        <img src={`https://source.unsplash.com/600x800/?construction,${index}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt={getText(item.name)}/>
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-8">
                                            <h4 className="text-3xl font-bold text-white mb-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">{getText(item.name)}</h4>
                                            <p className="text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity duration-300">{getText(item.description)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {isVisible('cta') && (
                    <section className="py-24 px-6 bg-gray-800 text-white">
                        <div className="container mx-auto text-center max-w-4xl">
                            ...
                            <button className="preview-primary-bg text-white font-bold py-4 px-10 rounded text-lg hover:opacity-90">{getText(c.cta.ctaButton)}</button>
                        </div>
                    </section>
                )}
                                ...
                                {c.testimonials.items.map((item, index) => (
                                    <blockquote key={index}>
                                        ...
                                        <footer className="mt-4">
                                            <cite className="font-semibold not-italic text-gray-900">{getText(item.author)}</cite>, <span className="text-gray-500">{getText(item.authorRole)}</span>
                                        </footer>
                                    </blockquote>
                                ))}
                            </div>
